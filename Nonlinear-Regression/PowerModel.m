...
%% Increase the linearity of the nonlinear data by Power model
 % This file is used to test the power model for the goal of achieve the
...
trainX_pow=log(trainX_n);
testX_pow=log(testX_n);
trainY_pow=log(trainY);
model=linregFit(trainX_pow,trainY_pow);
res=linregPredict(model,testX_pow);
res_t=linregPredict(model,trainX_pow);
